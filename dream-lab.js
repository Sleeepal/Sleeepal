...
function renderIdentity() {
  ...
  if (walletPreviewButton) {
    const availableHere = dreamApiOriginEligible();
    walletPreviewButton.textContent = availableHere ? "连接钱包账号" : "钱包账号 · 即将开放";
    walletPreviewButton.disabled = !availableHere;
  }
}
...
function enterCreateMode() {
  ...
  renderWorkspace();
  globalThis.setTimeout(() => wishInput?.focus({ preventScroll: true }), 180);
}
...
async function generateAccountArtifact(kind) {
  ...
  if (!authenticatedAccount) {
    ...
    walletPreviewButton?.focus({ preventScroll: true });
    return;
  }
...
function hardenOfflineHtml(content) {
  const policy = [
    "default-src 'none'",
    "base-uri 'none'",
    "connect-src 'none'",
  ...
  const withoutExistingPolicy = content.replace(
    /<meta\s+[^>]*http-equiv=["']Content-Security-Policy["'][^>]*>/gi,
    "",
  );
  if (/<head(?:\s[^>]*)?>/i.test(withoutExistingPolicy)) {
    return withoutExistingPolicy.replace(/<head(?:\s[^>]*)?>/i, (head) => `${head}${meta}`);
  }
  return `${meta}${withoutExistingPolicy}`;
}

function artifactDownloadPayload(result, fallbackVersion = 1) {
  const originalContent = typeof result.content === "string" ? result.content : "";
  if (!originalContent) throw new Error("产物内容为空，暂时无法使用。");
  const mimeType = normalizeText(result.artifact?.mimeType)
    || "text/plain;charset=utf-8";
  const content = mimeType.toLowerCase().startsWith("text/html")
    ? hardenOfflineHtml(originalContent)
    : originalContent;
  const safeFilename = (
...
function resetArtifactPreviewContent() {
  artifactPreviewFrame?.removeAttribute("srcdoc");
...
async function previewAccountArtifact(artifact) {
  ...
  try {
    ...
    if (artifactPreviewFrame) artifactPreviewFrame.srcdoc = payload.content;
    setMessage(
      artifactPreviewStatus,
      artifact.kind === "interactive_prototype"
        ? "隔离预览不保存状态，并阻断外部网络请求。下载后可在本机导出状态 JSON；系统不自动附加账号或服务标识，你填写的内容会随状态导出。"
        : "隔离预览不保存状态，并阻断外部网络请求。下载后可导出进度 JSON，再回到这里检查并明确确认导入。",
    );
...
async function connectWalletAccount() {
  ...
  try {
    ...
    accountActionBusy = false;
    if (walletPreviewButton) walletPreviewButton.disabled = !dreamApiOriginEligible();
  }
}
...
form?.addEventListener("submit", (event) => {
  ...
  if (!localConsent?.checked) {
    localConsent?.focus({ preventScroll: true });
...
messageForm?.addEventListener("submit", async (event) => {
  ...
  if (pending && content === pending.content) {
    ...
    aiModeButton?.focus({ preventScroll: true });
    return;
  }
...
progressClearButton?.addEventListener("click", () => {
  ...
  progressFileInput?.focus({ preventScroll: true });
});
